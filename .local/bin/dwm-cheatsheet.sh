...
get_lf_keybinds () {
    ...
    printf "\n%b\n" "${YELLOW}Shortcuts:${NC}"
    printf "${GREEN}%-30s ${BLUE}%-25s ${RED}%s${NC}\n" "Key" "Function" "Explanation"
    printf '%*s\n' "$(tput cols)" '' | tr ' ' '-'
	...
	/^map/ {
		...
		func = (func == "" ? $i : func " " $i);  # Simplified concatenation
	    }
	    printf "%s%-30s%s %-25s%s %s%s\n", "green", key, "blue", func, "red", comment, "nc";
	}
	' "$LF_CONFIG_PATH"
}
...
( print_welcome && get_constants && get_keybindings && get_shortcuts && get_lf_keybinds ) | less
